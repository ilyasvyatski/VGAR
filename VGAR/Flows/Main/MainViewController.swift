...
extension MainViewController {
    func renderer(_ renderer: SCNSceneRenderer, didAdd node: SCNNode, for anchor: ARAnchor) {
        ...
        // Загрузка видео
        loadVideo(for: imageAnchor.referenceImage.name!) { [weak self] videoSize in
            DispatchQueue.main.async {
                ...
                let planeVideo: SCNPlane
                
                if referenceImageName == "vg_1" {
                    let scaledHeight = referenceImageSize.height
                    let scaledWidth = (scaledHeight / videoHeight) * videoWidth
                    planeVideo = SCNPlane(width: scaledWidth, height: scaledHeight)
                } else {
                    planeVideo = SCNPlane(width: referenceImageSize.width, height: referenceImageSize.height)
                }
    ...
    func loadVideo(for target: String, completion: @escaping (CGSize) -> Void) {
...
////                 let videoHeight = videoSize.height
////                 
////                 // Проверяем, шире ли видео на 80% по сравнению с маркером
////                 let widthThreshold = referenceImageSize.width * 1.8
////                 
////                 let planeVideo: SCNPlane
////                 
////                 if videoWidth > widthThreshold {
////                     // Видео шире на 80% - масштабируем по высоте
////                     let scaledHeight = referenceImageSize.height //* 1.1 // Увеличиваем на 10%
////                     let scaledWidth = (scaledHeight / videoHeight) * videoWidth
////                     planeVideo = SCNPlane(width: scaledWidth, height: scaledHeight)
////                 } else {
////                     // Используем размеры маркера
////                     planeVideo = SCNPlane(width: referenceImageSize.width, height: referenceImageSize.height)
////                 }
////                 
////                 if let videoScene = self.videoScene() {
...
////    }
//
////    private func loadVideo(for target: String, completion: @escaping (CGSize) -> Void) {
////        DispatchQueue.global().async {
////            guard let videoURL = URL(string: "https://neoviso.com/arvideo/vg/\(target).mp4") else {
////                return
////            }
